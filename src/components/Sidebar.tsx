    ...
    <>
      ...
      >
        ...
        <div className="shrink-0 p-3 pb-6 space-y-2 border-t border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-900">
          ...
          {/* Theme toggle + Logout dalam satu baris */}
          <div className="flex gap-2">
            <button
              onClick={toggleTheme}
              className="flex items-center justify-center gap-1.5 flex-1 px-2 py-2 rounded-xl text-xs font-medium text-gray-600 dark:text-slate-300 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 transition-all"
              title={theme === "dark" ? "Mode Terang" : "Mode Gelap"}
            >
              {theme === "dark"
                ? <><Sun className="w-3.5 h-3.5 text-yellow-500" /> Terang</>
                : <><Moon className="w-3.5 h-3.5 text-indigo-500" /> Gelap</>
              }
            </button>
            <button
              onClick={() => setShowLogoutConfirm(true)}
              className="flex items-center justify-center gap-1.5 flex-1 px-2 py-2 rounded-xl text-xs font-semibold text-white bg-red-600 hover:bg-red-700 transition-all"
            >
